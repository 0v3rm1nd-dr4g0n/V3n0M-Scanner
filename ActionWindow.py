    ...
    def periodicCheckForMessages(self):
        ...
        try:
            while 1:
                ...
                if code == thread_Handler.MESSAGE_LOG:
                    self.onLogMessage(item)
                elif code == thread_Handler.MESSAGE_PROGRESS:
                    self.onProgress()
                elif code in (
                thread_Handler.MESSAGE_EXIT_CANCEL, thread_Handler.MESSAGE_EXIT_ERROR, thread_Handler.MESSAGE_EXIT_OK):
...
if __name__ == '__main__':
    ...
    t = thread_Handler.ThreadsConnector()
    w.setConnector(t)
    w.go()
